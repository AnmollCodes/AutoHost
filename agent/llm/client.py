...
def call_llm(prompt: str, force_json: bool = False) -> str:
    ...
    try:
        ...
        if force_json:
            if "ONLY valid JSON" not in prompt:
                prompt += "\n\nIMPORTANT: Output ONLY valid JSON starting with { and ending with }. No markdown blocks, no explanations, no thought tags."
...
def repair_json(text: str) -> dict[str, Any]:
    ...
    - Reasoning blocks (<think> tags from models like DeepSeek-R1 or Kimi)
    """

    # 0. Remove reasoning tags if present
    if "<think>" in text:
...
async def call_llm_async(prompt: str, force_json: bool = False) -> str:
    ...
    try:
        ...
        if force_json:
            # Use Ollama's native JSON mode for compatible models.
            # Some models may not support this - if errors occur, they'll be caught.
            if "ONLY valid JSON" not in prompt:
                prompt += "\n\nIMPORTANT: Output ONLY valid JSON starting with { and ending with }. No markdown blocks, no explanations, no thought tags."
            kwargs["prompt"] = prompt
            kwargs["format"] = "json"
        ...
        raise LLMError(
            ...
            f"Try increasing AUTOHOST_OLLAMA_TIMEOUT (current: {s.ollama_timeout}s)"
        )
        ...
        if "timeout" in error_str:
            ...
            raise LLMError(
                ...
                f"Try a smaller model like 'kimi-k2.5:cloud' or 'llama3.2:3b'"
            )
        raise LLMError(f"LLM request failed: {e}")
