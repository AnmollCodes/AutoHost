"""
File Permission Management for AutoHost.
...
# Additional sensitive file names (checked without path prefix)
...
def _is_sensitive_filename(path: str) -> bool:
    """Check if filename (not path) is sensitive."""
    path_obj = Path(path)
    return path_obj.name in SENSITIVE_FILE_NAMES or any(
        path_obj.name.startswith(f) for f in SENSITIVE_FILE_NAMES if f.startswith(".")
    )


def _expand_path(path: str) -> str:
    """Expand ~ and resolve to absolute path (preserves platform separators)."""
...
def check_path_access(path: str) -> AccessLevel:
    ...
    # First check: Always blocked sensitive patterns AND filenames
