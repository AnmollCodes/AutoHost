...
class ReActAgent:
    ...
    def __init__(
        ...
        require_confirmation: bool | None = None,  # If False, skip confirmation prompts
    ...
    async def run(self, goal: str) -> AgentState:
        ...
        if mem:
            memories = mem.retrieve(goal)
            if memories:
                logger.info("memories_retrieved", count=len(memories))
                state.context["memories"] = memories

        # Try to decompose into parallel subtasks (only for main agent)
        if not self._is_sub_agent:
            # Smart heuristic: Only run the planner for complex multi-step tasks
            # Simple questions/greetings/short requests skip planning entirely
            goal_lower = goal.lower().strip()
            is_simple = (
                len(goal) < 60
                or goal_lower.startswith(("hi", "hey", "hello", "what", "who", "how"))
                or goal.endswith("?")
                or goal_lower in ("/help", "/status", "/clear")
            )

            if not is_simple:
                # Use AI Task Planner for complex tasks
                plan = await TaskPlanner.create_plan(goal, self.conversation_history)
            ...
            # Only try parallel decomposition if planner didn't already produce a plan
            # AND the request is not a simple question making decomposition a waste of time
            if "active_plan" not in state.context and not is_simple:
                should_parallelize, subtasks = await self._should_decompose(goal)
            else:
                should_parallelize, subtasks = False, []
        ...
        if state.status == "running":
            ...
            # If we have a final answer from the last iteration, it's effectively completed
    ...
    async def _think(
        ...
        prompt = REACT_STEP_PROMPT.format(
            ...
            memories=memories_text or "No relevant past memories.",
    ...
    async def _execute_action(
        ...
        try:
            ...
            # Handle codebase analyzer
            if action.tool == "codebase_analyzer":
            ...
            # Handle intranet spidering
            if action.tool == "crawl_internal":
                url = action.args.get("url", "")
                max_pages = action.args.get("max_pages", 3)
                result = crawl_internal(url, max_pages)

                if result.get("error"):
                    return StepResult(
                        step_id="crawl_internal",
                        status="error",
                        error=result["error"],
                    )
            ...
            return StepResult(
                step_id=f"action_{action.tool}",
                status="error",
                error=f"Unknown tool: {action.tool}. Use 'shell', 'python', 'web_search', 'fetch_webpage', or 'crawl_internal'.",
            )
