/**
 * AutoHost Web UI Application
 * 
 * Main JavaScript application for the AutoHost web interface.
...
let ws = null;
let sessionId = localStorage.getItem('autohost_session') || null;
let isProcessing = false;
let currentTaskId = null;
let currentModel = localStorage.getItem('autohost_model') || 'kimi-k2.5:cloud';
...
function changeModel(model) {
    ...
    localStorage.setItem('autohost_model', model);
}
...
async function sendMessage() {
    ...
    try {
        ...
        localStorage.setItem('autohost_session', sessionId);
...
function addMessage(text, type, meta = null) {
    ...
    if ((type === 'assistant' || type === 'system') && typeof marked !== 'undefined') {
...
function clearChat() {
    ...
    sessionId = null;
    localStorage.removeItem('autohost_session');
}
...
function saveToHistory(request, response) {
    const history = JSON.parse(localStorage.getItem('autohost_history') || '[]');
    history.unshift({
        id: Date.now(),
    ...
    localStorage.setItem('autohost_history', JSON.stringify(history.slice(0, 20)));
    renderHistory();
}
