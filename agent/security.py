...
def validate_path(
    ...
    # Check for path traversal patterns in original string (before expansion)
    # This catches attempts like ../../../etc/passwd
    temp_path = Path(path_str)
    if ".." in temp_path.parts:
        raise PathTraversalError("Path traversal detected: contains '..'")
    ...
    for sensitive in unix_sensitive_paths:
        ...
        if (
            resolved_str.lower().endswith(sensitive_lower)
            or "/" + sensitive_lower.lstrip("/") in resolved_str.lower()
        ):
