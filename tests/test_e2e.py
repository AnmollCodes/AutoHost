...
class TestSandboxIntegration:
    ...
    async def test_python_file_operations(self, sandbox, temp_workspace):
        ...
        workspace_path = str(temp_workspace).replace("\\", "/")
        code = f"""
import os
files = os.listdir(r'{workspace_path}')
...
"""
    ...
    async def test_python_creates_file(self, sandbox, temp_workspace):
        ...
        # Use forward slashes and raw strings for cross-platform compatibility
        new_file_path = str(new_file).replace("\\", "/")
        code = f"""
with open(r'{new_file_path}', 'w') as f:
...
class TestRealE2E:
    ...
    async def test_real_python_file_creation(self, workspace):
        ...
        output_file_path = str(output_file).replace("\\", "/")
...
"""
    ...
    async def test_real_python_reads_file(self, workspace):
        ...
        code = f"""
with open(r'{test_file_path}', 'r') as f:
    content = f.read()
...
"""
    ...
    async def test_real_python_directory_listing(self, workspace):
        ...
        code = f"""
import os
for item in os.listdir(r'{workspace_path}'):
    print(item)
"""
        result = await sandbox.run_python(code, working_dir=str(workspace))
